...
import org.tpri.djcom.util.RequestUtils;
...
public class AuthValidateFilter implements Filter {
    ...
    public void init(FilterConfig fConfig) throws ServletException {
        ...
        if (!StringUtils.isBlank(pValue)) {
            ...
            for (int i = 0; i < this.ignoreUrls.length; i++) {
                ignoreUrls[i] = this.basePath + ignoreUrls[i];
            }
        }
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException,
     ...
     // 验证键值存在检查
        ...
        if (values.length > 2) {
            req.setAttribute(BaseConstants.REQ_CUR_ROLE_CODE, values[2]);
            if (values.length > 3) {
                req.setAttribute(BaseConstants.REQ_CUR_ORG_ID, values[3]);
            }
        }
        chain.doFilter(request, response);
    }
