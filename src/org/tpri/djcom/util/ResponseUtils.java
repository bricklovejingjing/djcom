...
public final class ResponseUtils {
    ...
    public static void setCookieValue(HttpServletResponse response, String name, String value) {
        ...
        String encryptValue = EncryptUtils.encrypt(value);
        Cookie cookie = new Cookie(name, encryptValue);
        cookie.setPath("/");
    ...
    public static void setValidateKey(HttpServletResponse response, String validateKey) {
        setCookieValue(response, BaseConstants.COOKIE_VALIDATE_KEY, validateKey);
    }
    ...
    public static void putHtmlResponse(HttpServletResponse response, String html) {
        response.setContentType("text/html;charset=" + BaseConstants.CHARSET_UTF8);
        response.setCharacterEncoding(BaseConstants.CHARSET_UTF8);
        try {
    ...
    public static void putTextResponse(HttpServletResponse response, String text) {
        response.setContentType("text/plain;charset=" + BaseConstants.CHARSET_UTF8);
        response.setCharacterEncoding(BaseConstants.CHARSET_UTF8);
        try {
    ...
    public static void putFileResponse(HttpServletRequest request, HttpServletResponse response, String fileName) {
        putFileResponse(request, response, fileName, null, BaseConstants.CHARSET_UTF8);
    }
     ...
     * @param inline 是否内嵌
     */
    public static void putFileResponse(HttpServletRequest request, HttpServletResponse response, String fileName, Boolean inline) {
        putFileResponse(request, response, fileName, inline, BaseConstants.CHARSET_UTF8);
    }
    
	/** 将文件数据写入输出流*/
    public static boolean writeFile(String filePath, HttpServletResponse response) {
        try {
     ...
     * @param charset 字符集
     */
    public static void putFileResponse(HttpServletRequest request, HttpServletResponse response, String fileName, Boolean inline, String charset) {
        ...
        try {
            if (RequestUtils.isFirefox(request)) {
                fileName = new String(fileName.getBytes(BaseConstants.CHARSET_UTF8), BaseConstants.CHARSET_LATIN);
            }
            else {
                fileName = URLEncoder.encode(fileName, BaseConstants.CHARSET_UTF8);
            }
        }
